...
to read a range profile that looks very similar (see figure \ref{fig:had_us}) to
...
Chapter \ref{idea} describes the geometry for 2D-cases. However, it
...
visualized in figure \ref{fig:2ddoa}. Horizontal DOA would be measured between
...
\begin{figure}[htp]
    \centering
    \label{fig:2ddoa}
    \includegraphics[max height=10cm,max width=10cm]{diagrams/2D-DOA}
...
\begin{figure}[htp]
    \centering
    \label{fig:3dcase}
    \includegraphics[max height=10cm,max width=10cm]{diagrams/3Dcase}
    \caption{A target detected at range \(R\) and relative Doppler speed corresponding to reprojection angle \(\alpha\) will be at a point on a circle (green) around the flight path (blue).}
\end{figure}
...
presented in the general 2D-case \ref{geometry-for-the-general-case} and 3D-case geometry (figure \label{fig:3dcase}) for a
